 ...
 * Stream aggregates work on streams like transaction logs , every record keeps it's value, like bank records 
    ...
    Exercises : 
    1. Make this ready to store the data in topic payment
    2. Make another streaming app that sends payments Rejected to topic FunctionalException 

 */
public class IdemPotent06Working {
    ...
    public static void main(final String[] args) {
        ...
        KStream<String, PaymentIntake> paymentIntakes = builder.stream(PaymentIntake.topicName,
                ...
                .aggregate(
                    ()-> new PaymentIntakeAggregate() //to initialize for every grouped item the total is zero
                    ,
                    (k,v,aggValue) ->
                        new PaymentIntakeAggregate( aggValue,v)
                    ,
                    Materialized.<String, PaymentIntakeAggregate, KeyValueStore<Bytes, byte[]>>as("PaymentIntakeAggregateStore")
                    .withKeySerde(AppSerdes.String())
                    .withValueSerde(AppSerdes.PaymentIntakeAggregate())
                ).toStream();
                
             analyse.mapValues((id,v)-> v.getUnique()).peek((id,paymentIntake)-> System.out.println(new Payment(paymentIntake, Payment.Status.ACCEPTED)));
             analyse.mapValues((id,v)-> v.getNotUnique()).peek((id,lst)-> {for (PaymentIntake paymentIntake: lst)System.out.println(new Payment(paymentIntake, Payment.Status.REJECTED));});
